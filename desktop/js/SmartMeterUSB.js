...
if (typeof SmartMeterUSBFrontEnd === "undefined") {
  var SmartMeterUSBFrontEnd = {
   ...
   */
  SmartMeterUSBFrontEnd.init = function () {
    /*
     * Change
     */
    document
      ...
      .addEventListener("change", function (event) {
        ...
        if (_target = event.target.closest(".eqLogicAttr[data-l1key=configuration][data-l2key=counterType]",)) {
          SmartMeterUSBFrontEnd.counterTypeChanged(_target)
          return
        }
      })
  }

  SmartMeterUSBFrontEnd.counterTypeChanged = function (target) {
    let counterType = target.jeeValue()
    domUtils.ajax({
      async: true,
      global: false,
      url: SmartMeterUSBFrontEnd.ajaxUrl,
      data: {
        action: "getImageForCounterType",
      ...
      success: function (data) {
        ...
        document
          .getElementById("img_counter")
          .setAttribute("src", data.result)
      },
    })
  }

  /* Fonction permettant l'affichage des commandes dans l'équipement */
  SmartMeterUSBFrontEnd.addCmdToTable = function (_cmd) {
    if (!isset(_cmd)) {
...
SmartMeterUSBFrontEnd.init()

/* Fonction permettant l'affichage des commandes dans l'équipement */
function addCmdToTable(_cmd) {
  SmartMeterUSBFrontEnd.addCmdToTable(_cmd)
}
