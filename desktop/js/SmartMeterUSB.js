  ...
  SmartMeterUSBFrontEnd.init = function() {
    ...
    */
    document.getElementById('div_pageContainer').addEventListener('change', function(event) {
        SmartMeterUSBFrontEnd.counterTypeChanged()
    })
  }  

  SmartMeterUSBFrontEnd.counterTypeChanged = function() {
    let _target = null
   
    if ( _target = event.target.closest('.eqLogicAttr[data-l1key=configuration][data-l2key=counterType]')) {
      let counterType = _target.jeeValue()
      ...
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          document.getElementById('img_counter').setAttribute('src',data.result)
        }
      })
    }
  }
