...
if (typeof SmartMeterUSBConfig === "undefined") {
  ...
  SmartMeterUSBConfig.init = function() {
    document.getElementById('div_SmartMeterUSBConfig').addEventListener('click', function(event) {
      let _target = null
    ...
    window['SmartMeterUSB_postSaveConfiguration'] = SmartMeterUSBConfig.saveAdapters
  }

  SmartMeterUSBConfig.addAdapter = function(adapter) {
    let adptr = '<div class="adapter col-lg-4 col-md-6 col-sm-12">'
    adptr += '<div  style="background-color:var(--bg-modal-color);margin-top:5px; padding:10px">'
  ...
  SmartMeterUSBConfig.saveAdapters = function() {
    let adapters = document.querySelectorAll('#adaptersContainer .adapter').getJeeValues('.adapterAttr')
    domUtils.ajax({
      async: true,
      global: false,
      url: SmartMeterUSBConfig.ajaxUrl,
